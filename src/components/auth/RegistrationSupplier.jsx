...
import { emailExistError, registerSuccess } from '../toastMessages/Messages';

const RegistrationSupplier = () => {

    // http://localhost:8001/api/supplier/createSupplierUser
    ...
    const { values, errors, handleSubmit, handleChange, handleBlur, touched } = useFormik({
        ...
        onSubmit: async (values) => {
            await axios.post(`http://localhost:8001/api/supplier/createSupplierUser`, values).then((res) => {
                if (res) {
                    if (res.status === 201) {
                        registerSuccess()
                    }
            ...
            <form onSubmit={handleSubmit} className=' flex flex-col gap-6'>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="gap-2 flex rounded-md p-1 flex-col">
                        ...
                        <Input onChange={handleChange} values={values.organization} onBlur={handleBlur} errors={errors.organization} touched={touched.organization} placeholder="Enter Organization name" name="organizationName" id="company" type="text" />
                    </div>
                    ...
                    <div className="flex lg:flex-row flex-col gap-5 justify-between">
                        <div className=" lg:w-1/2 w-full gap-2 h-auto rounded-md flex flex-col p-1 ">
                            ...
                            <Input onChange={handleChange} values={values.country} onBlur={handleBlur} errors={errors.country} touched={touched.country} placeholder="Enter country name" name="countryName" id="country" type="text" />
                        </div>
                        <div className=" lg:w-1/2 w-full gap-2 rounded-md flex flex-col p-1 ">
